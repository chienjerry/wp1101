...
export default (boardSize, mineNum) => {
    ...
    for(let x = 0; x < boardSize; x++){
        for(let y = 0; y < boardSize; y++){
            if (board[x][y].value!=="💣"){
                if ( (x>0)&&(y>0)){if (board[x-1][y-1].value==="💣"){board[x][y].value++}}
                if ((x>0)){if (board[x-1][y].value==="💣"){board[x][y].value++}}
                if ( (x>0)&&(y<boardSize-1)){if (board[x-1][y+1].value==="💣"){board[x][y].value++}}
                if ( (y>0)){if (board[x][y-1].value==="💣"){board[x][y].value++}}
                if ( (y<boardSize-1)){if (board[x][y+1].value==="💣"){board[x][y].value++}}
                if ( (x<boardSize)&&(y>0)){if (board[x+1][y-1].value==="💣"){board[x][y].value++}}
                if ( (x<boardSize)){if (board[x+1][y].value==="💣"){board[x][y].value++}}
                if ( (x<boardSize)&&(y<boardSize)){if (board[x+1][y+1].value==="💣"){board[x][y].value++}}
            }
        }
